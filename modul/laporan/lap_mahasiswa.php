...
$query=$mysqli->query("SELECT * FROM tb_mahasiswa ORDER BY nis");
$cek = $query->num_rows;
...
$pdf->Cell(6, 0.8, 'Mahasiswa', 1, 0, 'C');
...
while($lihat=$query->fetch_assoc()){
	...
	$pdf->Cell(6, 0.8, ucwords($lihat['nama_siswa']),1, 0, 'C');	
	$pdf->Cell(4, 0.8, tanggal($lihat['tgl_lahir']),1, 0, 'C');
...
$pdf->Output("laporan_bmahasiswa.pdf","I");
